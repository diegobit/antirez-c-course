...
struct tfctx; // Pre-define tfctx to help compiler solve recursive definition.
...
int compareStringObject(tfobj *a, tfobj *b) {
    ...
    if (cmp) {
        // Prefix is equal between a and b
 ...
 * of the element added to the list, if needed. */
...
tfobj *parseNumber(tfparser *parser) {
    ...
    int numlen = end-start; // pointers are numbers, I can subtract. I get ehe num of chars.
...
/* Pop the top element from the interpreter main stack, assuming it will
 * match 'type', otherwise NULL is returned. Also the function returns NULL
 * if the stack is empty.
 *
 * The reference counting of the popped object is not modified: it is assumed
 * that we just transfer the ownership from the stack to the caller. */
 ...
 * same name already exist, if gets replaced by the new one */
...
tfctx *createContext(void) {
    ...
    // registerUserFunction() #TODO: implement
    return ctx;
}
...
int callSymbol(tfctx *ctx, tfobj *word) {
    ...
    if (fe == NULL) return TF_ERR; /* TODO: should also set the error in the ctx */
    if (fe->user_func) {
...
int basicMathFunctions(tfctx *ctx, char *name) {
    ...
    /* TODO: better error check. CHeck types */
...
int main(int argc, char **argv) {
    ...
    /* Read the program inmemory, for later parsing. */
    ...
    prgtext[file_size] = 0; // set null string
    fclose(fp);

    printf("Program Text before compilation:\n'%s'\n", prgtext);
